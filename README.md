...
### 项目难点
* 优化页面加载性能，减少首次加载时间，提升用户体验
* 实现复杂的商品详情页交互，如加入商品数量的最大值、最小值判断，商品保障服务、商品评价等弹出窗口

### 项目亮点
* 使用路由懒加载，对不同页面的代码进行分割和异步加载，减少首次加载时间
* 将查询参数转为JSON进行传递，解决刷新页面时，查询参数丢失对象类型数据的问题
* 使用Vuex管理应用状态，通过缓存机制减少了页面的加载时间，优化了全局状态的性能消耗
* 使用TypeScript进行编写，对状态类型进行约束，提高了开发效率
